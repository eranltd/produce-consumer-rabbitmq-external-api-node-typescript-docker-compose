...
export class StreetsService{
	...
	private static _resourceId = '1b14e41c-85b3-4c21-bdce-9fe48185ffca' 
	...
	static async getStreetsInCity(city: city): Promise<{city: city, streets: Pick<Street, 'streetId' | 'street_name'>[]}>{
		const res = (await this.axios.post('https://data.gov.il/api/3/action/datastore_search', {resource_id: this._resourceId, filters: {city_name: cities[city]}, limit: 100000})).data
	...
	static async getStreetInfoById(id: number){
		const res = (await this.axios.post('https://data.gov.il/api/3/action/datastore_search', {resource_id:this._resourceId, filters: {_id: id}, limit: 1})).data
		const results = res.result.records
		if (!results || !results.length) {
